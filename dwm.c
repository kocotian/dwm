...
{
	strncpy(m->attsymbol, directions[m->attachdirection].symbol, strlen(directions[m->attachdirection].symbol) + 1);
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	if ((((nexttiled(c->mon->clients) == c && !nexttiled(c->next))
	    || &monocle == c->mon->lt[c->mon->sellt]->arrange)
	    && !c->isfullscreen && !c->isfloating
	    && NULL != c->mon->lt[c->mon->sellt]->arrange) || c->isfloating) {
		if (c->isfloating) {
			c->w = wc.width;
			c->h = wc.height;
		} else {
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating) {
		if (arg != NULL) {
			if (arg->i)
				resize(selmon->sel, selmon->wx + 250, selmon->wy + 150,
					selmon->ww - 500, selmon->wh - 300, 0);
			else
